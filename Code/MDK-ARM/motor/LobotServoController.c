...
void moveServo(uint8_t servoID, uint16_t Position, uint16_t Time)
{
	...
	LobotTxBuf[9] = GET_HIGH_BYTE(Position);  //取得目标位置的高八位
	 ...
	 HAL_UART_Transmit(&huart2,LobotTxBuf, 10,100);  
}
...
void moveServosByArray(LobotServo servos[], uint8_t Num, uint16_t Time)
{
    ...
    HAL_UART_Transmit(&huart2,LobotTxBuf, LobotTxBuf[2] + 2 ,100);  
...
void moveServos(uint8_t Num, uint16_t Time, ...)
{
	...
	va_end(arg_ptr);  //置空arg_ptr
	 ...
	 HAL_UART_Transmit(&huart2,LobotTxBuf, LobotTxBuf[2] + 2 ,100);     //发送
}
...
void runActionGroup(uint8_t numOfAction, uint16_t Times)
{
	...
	HAL_UART_Transmit(&huart2, LobotTxBuf, 7,1000);
}
...
void stopActionGroup(void)
{
	...
	HAL_UART_Transmit(&huart2,LobotTxBuf, 4,100);      //发送
}
/*********************************************************************************
...
void setActionGroupSpeed(uint8_t numOfAction, uint16_t Speed)
{
	...
	HAL_UART_Transmit(&huart2, LobotTxBuf, 7,100);            //发送
}
...
//	uint16_t Voltage = 0;
	...
	HAL_UART_Transmit(&huart2,LobotTxBuf, 4,100);  
}
